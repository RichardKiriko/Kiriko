...
function merge_ass(subs)
	...
	if #input_files > 0 and output_file ~= "" then
		...
		if #PlayResX ~= 1 or #PlayResY ~= 1 then
			playres_gui={
						{class="label",x=1,y=0,width=1,height=1,label="脚本分辨率不一致，请设定脚本分辨率"},
						{class="edit",x=1,y=1,width=3,height=1,name="resx",value=PlayResX[1] or 1280},
						{class="label",x=4,y=1,width=1,height=1,label="×"},
						{class="edit",x=5,y=1,width=3,height=1,name="resy",value=PlayResX[1] or 720}
						}
			playres_r,playres =_G.aegisub.dialog.display(playres_gui)
		...
		for k,file_tbl in ipairs(files_tbl) do
			for j,style in ipairs(file_tbl.styles) do
				...
				elseif res == 2 then
					...
					table.insert(all_styles,{name=r_name,text=style.text})
				end
			end
		end
